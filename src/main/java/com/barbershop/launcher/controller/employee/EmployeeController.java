...
public class EmployeeController implements GenericControllerViewFunctions {
    ...
    private void loadEmployeeActivityStats(int totalCount) {
        ...
        List<Label> labels = List.of(active_employees_count, inactive_employees_count, active_employees_percentage, inactive_employees_percentage);

        List<String> texts = List.of(
                parseNumberValueToText(activeEmployeesCount),
                parseNumberValueToText(inactiveEmployeesCount),
                formatAsPercentage(activePercentage),
                formatAsPercentage(inactivePercentage)
        );
    ...
    private void configureEmployeeLiveSearch() {

        search_field.textProperty().addListener((_, _, _) -> executeEmployeeLiveSearch());
        status_filter.valueProperty().addListener((_, _, _) -> executeEmployeeLiveSearch());
        hire_date_filter.valueProperty().addListener((_, _, _) -> executeEmployeeLiveSearch());
    }

    private void executeEmployeeLiveSearch() {
